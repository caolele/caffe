...
void PatchTransformerLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*> &bottom,
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    ...
    for (int blob_id = 0; blob_id < bottom.size(); ++blob_id) {
      ...
      if (binarize.Get(blob_id)) {
        ...
        for (int c = 0; c < bottom[blob_id]->channels(); ++c) {
          ...
          cv::threshold(tmp, slice[c], 0.1, 1, cv::THRESH_BINARY);
        }
    ...
    // Lele: verify if input pairs are correct
    /*int dim = imgs[0].cols;
    cv::Mat showImg = cv::Mat::zeros(cv::Size(2 * dim, dim), CV_8UC3);
    imgs[0].copyTo(showImg(cv::Rect(0, 0, dim, dim)));
    cv::Mat lbltmp = imgs[1].clone() * 255;
    cv::Mat dst;
    cv::cvtColor(lbltmp, dst, CV_GRAY2RGB);
    dst.copyTo(showImg(cv::Rect(dim, 0, dim, dim)));
    time_t seconds;
    time(&seconds);
    std::stringstream ss;
    ss << "./tmp/" << seconds << ".jpg";
    imwrite(ss.str(), showImg);*/
